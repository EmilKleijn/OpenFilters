...
Q_function_descriptions = ["sqrt(R/T)", "sqrt(0.5*(1/T-T))", "0.5*ln((1+sqrt(R))/(1-sqrt(R)))",
                           "Microw. Opt. Techn. Let., vol. 22, pp. 140-144"]
...
class optimization_Fourier(optimization):
    ...
    def __init__(self, filter, targets, parent=None):
        ...
        for i in range(len(self.targets)):
            ...
            raise optimization_error(
                "The Fourier transform method needs a reflection spectrum target at normal incidence")
    ...
    def calculate_Q_function(self, R, phi):
        ...
        if self.Q_function == "Delano":
            ...
            for i in range(self.nb_wvls):
                ...
                A = math.log((math.sqrt(one_minus_R * one_minus_R + 4.0 * R[i] * sin_phi * sin_phi)) / (
                            1.0 - 2.0 * sqrt_R * math.cos(phi[i]) + R[i]))
    ...
    def iterate_(self):
        ...
        for i_x in range(nb_x):
            ...
            for i_wvl in range(self.nb_wvls):
                if Q_m[i_wvl] != 0.0:
                    integral += (Q_m[i_wvl] / k_corrected[i_wvl]) * math.sin(
                        Psi_m[i_wvl] - k_corrected[i_wvl] * 2.0 * (x - 0.5 * self.OT)) * dk[i_wvl]
        ...
        if new_chi_2 <= self.chi_2:
            ...
            self.depth, self.index_profile = graded.steps_to_index_profile(self.step_thickness, self.step_profile,
                                                                           self.steps)
    ...
    def copy_to_filter(self):
        ...
        if self.index_profile != []:
            self.filter.add_graded_layer_from_steps_with_material_nb(self.material_nb, self.step_profile,
                                                                     self.step_thickness)
